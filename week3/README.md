...
2. **实验 4（量价关系）**：分档（<100 / 100-1000 / 1000-10000 / >10000 USD）分析 `report/figures` 给出量价曲线。
3. **实验 5（revert 与逆向选择）**：用 `oracle_updates.status` 标记前序改价失败的 swap，对比毒性订单比例与事后价格漂移。
4. **扩窗**：整合 7 天数据后重跑实验 1 / 3 / 4 / 5，并考虑引入多 pool PDA 覆盖。
